...
class QuickAddShiftPage extends StatefulWidget {
  const QuickAddShiftPage({
    ...
    this.existingShifts = const <Shift>[],
  });
...
class _QuickAddShiftPageState extends State<QuickAddShiftPage> {
  ...
  QuesturaOfficeProfile _questuraOfficeProfile = QuesturaOfficeProfile.sixHours;
  ...
  OvertimeDestination _overtimeDestination = OvertimeDestination.payment;

  bool _ordinaryHoursOverrideEnabled = false;

  TimeOfDay _ordinaryHoursOverrideTime = const TimeOfDay(hour: 6, minute: 0);
  ...
  void initState() {
    ...
    final baseServiceDate = widget.initialShift?.serviceDate ??
        widget.initialDate ??
        DateTime.now();
    ...
    _manualExtraAmountController = TextEditingController(
      text:
          cleanedManualAmount > 0 ? cleanedManualAmount.toStringAsFixed(2) : '',
    );
    ...
    _overtimeDestination =
        initialShift?.overtimeDestination ?? OvertimeDestination.payment;
    ...
    _polferTerritoryControlType = initialShift?.polferTerritoryControlType ??
    ...
    _questuraMode = initialShift?.questuraMode ?? QuesturaMode.uffici;

    _questuraPreset = initialShift?.questuraPreset ?? QuesturaPreset.none;

    _questuraOfficeProfile =
        initialShift?.questuraOfficeProfile ?? QuesturaOfficeProfile.sixHours;
    ...
    _includeAutostradaService = initialShift?.hasAutostradaService ?? false;
  ...
  DateTime? _buildQuesturaProgrammedOvertimeStart() {
    ...
    switch (_questuraProgrammedOvertimePreset) {
      case QuesturaProgrammedOvertimePreset.morning912:
        return DateTime(
            _serviceDate.year, _serviceDate.month, _serviceDate.day, 9);
      case QuesturaProgrammedOvertimePreset.afternoon1518:
        return DateTime(
            _serviceDate.year, _serviceDate.month, _serviceDate.day, 15);
      case QuesturaProgrammedOvertimePreset.evening2023:
        return DateTime(
            _serviceDate.year, _serviceDate.month, _serviceDate.day, 20);
      case QuesturaProgrammedOvertimePreset.none:
        return null;
    }
  }
  ...
  DateTime? _buildQuesturaProgrammedOvertimeEnd() {
    ...
    switch (_questuraProgrammedOvertimePreset) {
      case QuesturaProgrammedOvertimePreset.morning912:
        return DateTime(
            _serviceDate.year, _serviceDate.month, _serviceDate.day, 12);
      case QuesturaProgrammedOvertimePreset.afternoon1518:
        return DateTime(
            _serviceDate.year, _serviceDate.month, _serviceDate.day, 18);
      case QuesturaProgrammedOvertimePreset.evening2023:
        return DateTime(
            _serviceDate.year, _serviceDate.month, _serviceDate.day, 23);
      case QuesturaProgrammedOvertimePreset.none:
        return null;
    }
  }
  ...
  DateTime? _buildCustomProgrammedOvertimeEnd() {
    ...
    final startMinutes = _programmedOvertimeStartTime.hour * 60 +
        _programmedOvertimeStartTime.minute;
    final endMinutes = _programmedOvertimeEndTime.hour * 60 +
  ...
  Shift _buildShiftPreview() {
    ...
    if (_hasAbsence) {
      ...
      return Shift(
        ...
        compensativeRecoveryHours: _selectedAbsence == 'Recupero compensativo'
    ...
    return Shift(
      ...
      genereDiConfortoCdg: widget.activeDepartment == Department.repartoMobile
          ? _includeGenereDiConfortoCdg
          : false,
      genereDiConforto: widget.activeDepartment == Department.repartoMobile
          ? _includeGenereDiConforto
          : false,
      ticketPasto: _includeTicketPasto,
      overtimeDestination: _overtimeDestination,
      compensativeOvertimeHours: _usesCompensativeOvertime
          ? _parseDouble(_compensativeOvertimeHoursController.text)
          : 0.0,
      compensativeOvertimeNote: _usesCompensativeOvertime
          ? _compensativeOvertimeNoteController.text.trim()
          : '',
      ordinaryHoursOverrideEnabled: _ordinaryHoursOverrideEnabled,
      ordinaryHoursOverride: _ordinaryHoursOverrideEnabled
          ? _timeOfDayToDecimalHours(
              _ordinaryHoursOverrideTime,
            )
          : 0.0,
      programmedOvertimeEnabled: _programmedOvertimeEnabled ||
          (_usesQuesturaPresetLogic &&
              _questuraMode == QuesturaMode.volanti &&
              _questuraProgrammedOvertimePreset !=
                  QuesturaProgrammedOvertimePreset.none),
      programmedOvertimeStart: _programmedOvertimeEnabled
          ? _buildCustomProgrammedOvertimeStart()
          : _buildQuesturaProgrammedOvertimeStart(),
      programmedOvertimeEnd: _programmedOvertimeEnabled
          ? _buildCustomProgrammedOvertimeEnd()
          : _buildQuesturaProgrammedOvertimeEnd(),
      programmedOvertimeNote: _programmedOvertimeEnabled
          ...
          : (_usesQuesturaPresetLogic &&
                  ...
                  _questuraProgrammedOvertimePreset !=
                      QuesturaProgrammedOvertimePreset.none
              ? (_isPolstrada
                  ? 'Straordinario programmato Pattuglia'
      ...
      questuraOfficeProfile: _usesQuesturaPresetLogic
          ? _questuraOfficeProfile
          : QuesturaOfficeProfile.sixHours,
      questuraOfficeOrdinaryHours:
          _usesQuesturaPresetLogic ? _questuraOfficeOrdinaryHours() : 6.0,
  ...
  String? _validateBeforeSave(Shift previewShift) {
    ...
    if (_isPolfer && _hasPolferScalo && _polferScaloManualOverride) {
      ...
      final programmedOvertimeHours = previewShift.programmedOvertimeEnabled &&
  ...
  bool _isSameServiceDate(DateTime first, DateTime second) {
    return first.year == second.year &&
        first.month == second.month &&
        first.day == second.day;
  }

  List<Shift> _buildPreviewDayShifts(Shift previewShift) {
    final existingDayShifts = widget.existingShifts.where((shift) {
      final isOriginalShiftBeingEdited =
          _isEditing && identical(shift, widget.initialShift);

      if (isOriginalShiftBeingEdited) {
        return false;
      }

      return _isSameServiceDate(
        shift.serviceDate,
        previewShift.serviceDate,
      );
    });
  ...
  List<Map<String, dynamic>> _buildEnginePreviewBreakdown(Shift previewShift) {
    ...
    final result = useCase.execute(
      shifts: _buildPreviewDayShifts(previewShift),
                      ...
                      IgnorePointer(
                        ...
                        child: Opacity(
                          ...
                          child: _ModernSwitchTile(
                            ...
                            title: 'Servizio esterno',
                            subtitle:
                                'Attivalo quando il servizio viene svolto esternamente e spetta l’indennità.',
                            onChanged: (value) {
                              setState(() {
                                _externalService = value;
                              });
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              if (_usesCompensativeOvertime) ...[
                                ...
                                TextField(
                                  controller:
                                      _compensativeOvertimeHoursController,
                                  keyboardType:
                                ...
                                TextField(
                                  controller:
                                      _compensativeOvertimeNoteController,
                                  decoration: const InputDecoration(
                              ...
                              SwitchListTile(
                                ...
                                onChanged: (value) {
                                  setState(() {
                                    ...
                                    if (value) {
                                      if (_ordinaryHoursOverrideController.text
                                          .trim()
                                          .isEmpty) {
                                        _ordinaryHoursOverrideController.text =
                                            '6';
                                      }
                                      _ordinaryHoursOverrideNoteController
                                              .text =
                    ...
                    DropdownButtonFormField<String>(
                      ...
                      onChanged: (value) {
                        ...
                        setState(() {
                          ...
                          _selectedSpmnPreset = value == 'Riposo'
                              ? SpmnPreset.riposo
                              : SpmnPreset.none;
                    ...
                    if (breakdown.isEmpty && informativeItems.isEmpty)
                      Container(
                        ...
                        decoration: BoxDecoration(
                          ...
                          borderRadius: BorderRadius.circular(18),
                          border:
                              Border.all(color: _QuickAddPalette.cardBorder),
                        ),
                        child: Text(
                          _hasAbsence
